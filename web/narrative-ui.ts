import { Project, Scene, Event, FactChange } from "../src/model.js";
import {
  allScenes,
  universeReadiness,
  sceneLocation,
...
import { wordCount } from "../src/project.js";
...
const button = (
  ...
  `<button type="button" class="btn ${cls}" data-action="${action}" data-id="${e(id)}">${e(label)}</button>`;
...
const name = (p: Project, id: string) =>
  p.entities.find((x) => x.id === id)?.name ?? id;
const chip = (text: string, cls = "") =>
  `<span class="tag ${cls}">${e(text)}</span>`;

export function guide(p: Project, gate = false): string {
...
export function sceneLinks(p: Project, s: Scene): string {
  const arcs = sceneArcs(p, s),
    events = sceneEvents(p, s),
    place = sceneLocation(p, s);
  const people = (s.participants ?? []).filter((id) =>
    p.entities.some((en) => en.id === id && en.kind === "character"),
  );
...
export function sceneBoard(p: Project): string {
  const all = allScenes(p);
  return `<div class="page-heading"><div><p class="eyebrow">LE CHEMIN DU RÉCIT</p><h1>Composez vos scènes.</h1><p>Une carte, un moment, une intention. Ouvrez une scène pour écrire.</p></div>${button("+ Ajouter un chapitre", "new-chapter", "", "primary")}</div><div class="narrative-legend">${chip(`${all.length} scènes`)}${chip(`${p.arcs.length} intrigues`)}${chip(`${p.events.length} événements`)}<span>Ordre de lecture → · Les moments indiquent l’ordre réel des faits.</span></div><div class="scene-board">${
    p.chapters
      .map(
        (c, i) =>
          `<section class="chapter-lane"><header><div><span class="eyebrow">CHAPITRE ${String(i + 1).padStart(2, "0")}</span><h2>${e(c.title)}</h2></div>${button("Modifier", "edit-chapter", c.id, "quiet small")}</header>${c.brief ? `<p class="muted small">${e(c.brief)}</p>` : ""}<div class="scene-stack">${
            c.scenes
              .map((s) => {
                const index = all.indexOf(s),
                  previous = all[index - 1],
                  flashback = previous && s.storyTime < previous.storyTime;
                const arcs = sceneArcs(p, s),
                  events = sceneEvents(p, s),
                  place = sceneLocation(p, s);
                return `<article class="scene-card"><div class="section-head"><span class="eyebrow">SCÈNE ${index + 1}</span>${chip(flashback ? `↶ Retour · ${s.storyTime}` : `Moment ${s.storyTime}`, flashback ? "plan" : "")}</div><button class="scene-card-title" data-action="select-scene" data-id="${e(s.id)}">${e(s.title || "Sans titre")}</button><p class="scene-card-plan">${e(s.plan || "Quelle est l’intention de cette scène ?")}</p><div class="scene-card-world"><span>⌖ ${e(place?.name ?? "Choisir un lieu")}</span><span>${e(
                  (s.participants ?? [])
                    .filter((id) =>
                      p.entities.some(
                        (en) => en.id === id && en.kind === "character",
                      ),
                    )
                    .map((id) => name(p, id))
                    .join(" · ") || "Choisir les personnages",
                )}</span></div><div class="scene-card-arcs">${arcs.map((id) => chip(p.arcs.find((a) => a.id === id)?.title ?? id, "plot-tag")).join("") || chip("Intrigue à relier", "unlinked")}${s.beat ? chip(beats[s.beat]) : ""}</div><div class="card-progress"><span>${s.status === "complete" ? "● Terminée" : "○ En écriture"}</span><span>${wordCount(s.text ?? "")} mots · ${events.length} événement(s)</span></div><div class="scene-card-actions">${button("Écrire →", "select-scene", s.id, "primary small")}${button("Relier", "edit-scene", s.id)}${button("Supprimer", "delete-scene", s.id, "quiet small danger")}</div></article>`;
              })
              .join("") ||
            '<p class="empty-inline">Le prochain moment de votre histoire.</p>'
          }</div>${button("+ Ajouter une scène", "new-scene", c.id, "add-scene-button")}</section>`,
      )
      .join("") ||
    "<p>Ajoutez votre premier chapitre pour organiser le récit.</p>"
  }</div><aside class="tip"><p><strong>Une scène fait avancer une question.</strong> Reliez-la à une intrigue, indiquez son rôle, puis déclarez les changements importants. Un retour en arrière change son moment, pas sa place dans le livre.</p></aside>`;
}

export function plotMap(p: Project): string {
  return `<section class="plot-map"><div class="section-head"><div><p class="eyebrow">VOS FILS NARRATIFS</p><h2>De la promesse aux conséquences.</h2></div>${button("Voir le tableau des scènes", "writing-board")}</div><p class="muted small">Reliez des scènes à chaque intrigue. Les événements de ces scènes apparaissent automatiquement dans ce fil ; leur statut reste indépendant.</p>${
    p.arcs
      .map((a, i) => {
        const scenes = allScenes(p).filter((s) =>
          sceneArcs(p, s).includes(a.id),
        );
      ...
      .join("") ||
    '<p class="empty-inline">Créez une intrigue, puis reliez les scènes qui la font progresser.</p>'
  }</section>`;
}

export function changeText(p: Project, ch: FactChange): string {
  const value =
    ch.value === true
      ? "oui"
      : ch.value === false
...
export function narrativeTimeline(
  ...
  const range = (c: Project["chapters"][number]) =>
    c.scenes.length
      ? [
  ...
  if (order === "story") chapters.sort((a, b) => range(a)[0] - range(b)[0]);
  const events = p.events.filter(
    (ev) =>
      ev.milestone && (filter === "all" || (ev.status ?? "canon") === filter),
  );
  const cell = (content: string, cls = "") =>
    `<div class="time-cell ${cls}">${content}</div>`;
  ...
  return `<section class="timeline-studio" aria-label="Frise des chapitres et intrigues"><div class="timeline-controls"><div><p class="eyebrow">LA TABLE DU TEMPS · VUE MACRO</p><h2>Le récit dans son ensemble.</h2></div><div class="segmented" aria-label="Ordre de la frise">${button("Temps de l’histoire", "timeline-order", "story", order === "story" ? "active" : "")}${button("Ordre de lecture", "timeline-order", "reading", order === "reading" ? "active" : "")}</div><label class="timeline-zoom">Zoom <input id="timeline-zoom" type="range" min="0.7" max="1.7" step="0.1" value="${zoom}" aria-label="Zoom de la frise"></label></div><p class="timeline-help">Chapitres, fils narratifs et événements marquants. ${order === "story" ? "Les chapitres sont classés par leur premier moment ; leurs périodes peuvent se chevaucher." : "Les chapitres suivent l’ordre du livre, même lorsqu’ils contiennent un flash-back."} Les distances ne représentent pas les durées.</p><div class="timeline-navigation">${button("← Parcourir", "pan-timeline", "-1")}${button("Parcourir →", "pan-timeline", "1")}<span>Ouvrez un chapitre pour descendre au niveau des scènes.</span></div><div class="timeline-scroll" tabindex="0" aria-label="Frise défilante"><div class="timeline-grid" style="grid-template-columns:170px repeat(${Math.max(1, chapters.length)},${width}px)"><div class="time-label time-header">CHAPITRES</div>${chapters
    ...
    .map(
      (a, i) =>
        `<div class="time-label arc-label"><small>INTRIGUE ${i + 1}</small><strong>${e(a.title)}</strong></div>${chapters
          .map((c) => {
            const linked = c.scenes.filter((s) =>
              sceneArcs(p, s).includes(a.id),
            );
            const roles = [
              ...new Set(
                linked.flatMap((s) => (s.beat ? [beats[s.beat]] : [])),
              ),
            ];
            return cell(
              linked.length
                ...
                : '<span class="arc-gap" aria-label="Aucun lien déclaré"></span>',
              "arc-time-cell",
            );
          })
          .join("")}`,
    ...
    .join(
      "",
    )}<div class="time-label event-label"><small>CONTINUITÉ</small><strong>Jalons marquants</strong></div>${chapters
    .map((c) =>
      cell(
        events
          ...
          .map(
            (ev) =>
              `<button class="time-event ${ev.status ?? "canon"}" data-action="edit-event" data-id="${e(ev.id)}"><small>${stages[ev.status ?? "canon"]} · moment ${ev.storyTime}</small><strong>${e(ev.title)}</strong></button>`,
          )
          .join(""),
      ),
    )
