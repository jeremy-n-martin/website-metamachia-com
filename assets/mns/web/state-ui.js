...
export function effectDescription(t, effect) {
    const name = (id) => t.characters.find((c) => c.id === id)?.name || "Personnage";
...
export function choiceField(label, name, choices, customLabel, selected = "") {
...
export function inventoryFields(t, person = "", operation = "gain", initial = false) {
...
export function propertyFields(t, person = "", initial = false) {
    const names = [
        ...new Set([
            ...PROPERTIES,
...
export function worldHtml(t, count) {
    const state = stateAt(t, count), all = entries(t), name = (id) => t.characters.find((c) => c.id === id)?.name || "Personnage";
...
export function consequenceHtml(t, blockId) {
    const index = entries(t).findIndex((e) => e.block.id === blockId), b = entries(t)[index]?.block;
    if (!b?.effects.length)
        return "";
    const before = stateAt(t, index), after = stateAt(t, index + 1);
    const summary = (state, person) => {
    ...
    const people = [
        ...new Set(b.effects.flatMap((f) => f.type === "relation"
            ? [f.from, f.to]
    ...
    return `<details class="consequence-preview"><summary>Avant → après ce passage</summary>${people.map((p) => `<h4>${e(t.characters.find((c) => c.id === p)?.name)}</h4><p><b>Avant :</b> ${e(summary(before, p))}</p><p><b>Après :</b> ${e(summary(after, p))}</p>`).join("")}${after.issues
        ...
        .join("")}<p class="muted tiny">Les relations et appartenances sont visibles dans le panneau de gauche après lecture du passage.</p></details>`;
}
