...
const icons = {
    ...
    world: '<circle cx="12" cy="12" r="9"/><path d="M3 12h18M12 3c5 5 5 13 0 18-5-5-5-13 0-18"/>',
...
const icon = (name) => `<svg aria-hidden="true" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.6" stroke-linecap="round" stroke-linejoin="round">${icons[name] ?? icons.book}</svg>`;
const btn = (label, action, cls = "", attrs = "") => `<button type="button" class="btn ${cls}" data-action="${action}" ${attrs}>${label}</button>`;
const tag = (label, kind = "") => `<span class="tag ${kind}">${e(label)}</span>`;
const field = (label, name, value, type = "text", extra = "") => `<label class="field">${label}<input name="${name}" type="${type}" value="${e(value)}" ${extra}></label>`;
const area = (label, name, value, extra = "") => `<label class="field">${label}<textarea name="${name}" ${extra}>${e(value)}</textarea></label>`;
const option = (value, label, current) => `<option value="${e(value)}" ${value === current ? "selected" : ""}>${e(label)}</option>`;
const select = (label, name, options) => `<label class="field">${label}<select name="${name}">${options}</select></label>`;
const kinds = {
...
const date = (s) => new Date(s).toLocaleDateString("fr-FR", { day: "numeric", month: "short" });
const empty = (title, body, button = "") => `<div class="empty">${icon("spark")}<h3>${title}</h3><p>${body}</p>${button}</div>`;
function markdown(text) {
    return text
        ...
        if (/^> /.test(block))
            return `<blockquote>${safe.slice(5)}</blockquote>`;
...
try {
    ...
    saveError = `Lecture impossible : ${error.message} Vos données ne seront pas écrasées. Téléchargez la sauvegarde de secours.`;
    store = undefined;
}
function project() {
...
const filename = (p = project()) => p.title
    .normalize("NFD")
    .replace(/[\u0300-\u036f]/g, "")
    .replace(/[^a-zA-Z0-9-]/g, "-")
    .replace(/-+/g, "-")
    .slice(0, 70) || "histoire";
function openDialog(title, body, submitLabel, onSave) {
    modalSave = onSave;
...
function createStory() {
    openDialog("Une nouvelle histoire", field("Titre de l’histoire", "title", "", "text", 'required maxlength="160" autofocus placeholder="Le titre de votre prochaine aventure"') +
        field("Genre / ambiance", "genre", "", "text", 'placeholder="Fantasy, polar, récit de vie…"'), "Créer mon histoire", (data) => {
...
function universe() {
    const p = project();
    const items = p.entities.filter((en) => (worldFilter === "all" || en.kind === worldFilter) &&
...
function plot() {
    ...
    return `${pageHead("LE FIL ROUGE", "Tissez votre intrigue.", "Gardez vos promesses narratives, vos secrets et vos idées à portée de main.", btn(icon("plus") + "Intrigue", "new-arc", "primary") + btn(icon("plus") + "Note", "new-note", "secondary"))}${plotMap(p)}<details class="plot-status-board"><summary>Tableau de progression des intrigues</summary><div class="plot-board">${[
        ...
        .map((st) => `<section class="plot-lane"><div class="section-head"><h3><span class="lane-dot ${st}"></span>${statuses[st]}</h3>${tag(String(p.arcs.filter((a) => a.status === st).length))}</div>${p.arcs
...
function editChapter(id) {
    ...
    openDialog(c ? "Le chapitre" : "Un nouveau chapitre", field("Titre", "title", c?.title ?? `Chapitre ${p.chapters.length + 1}`, "text", "required maxlength=160") +
        area("Brief du chapitre", "brief", c?.brief ?? "", 'rows="5" placeholder="Objectif, ton, révélations, contraintes…"') +
        (c
            ...
            : ""), c ? "Enregistrer" : "Créer le chapitre", (data) => {
        modal.close();
        mutate(() => {
            if (c) {
                ...
                const sc = newScene(Math.max(0, ...p.chapters.flatMap((c) => c.scenes.map((s) => s.storyTime))) + 1);
                p.chapters.push({
                    id: uid("CH"),
    ...
    openDialog("Préparer et relier la scène", field("Titre", "title", s.title, "text", "required") +
        ...
        mutate(() => {
            ...
            if (dest !== c) {
                ...
                dest.scenes.push(s);
                p.events.forEach((ev) => {
                    if (ev.source?.scene === id)
                        ev.source.chapter = dest.id;
                });
            }
        });
    });
...
function editEntity(id, preset = "character") {
    ...
    openDialog(en ? "Une présence dans votre monde" : "Une nouvelle fiche", field("Nom", "name", en?.name ?? "", "text", "required maxlength=160") +
        ...
        (en
            ? btn("Supprimer cette fiche", "delete-entity", "danger quiet", `data-id="${id}"`)
            : ""), "Enregistrer la fiche", (data) => {
        modal.close();
        mutate(() => {
            const value = {
            ...
            if (en)
                Object.assign(en, value);
            else
                p.entities.push({ id: uid("ENT"), ...value });
        });
    });
}
...
function editArc(id) {
    ...
    openDialog(a ? "Le fil d’une intrigue" : "Une nouvelle intrigue", field("Titre / question narrative", "title", a?.title ?? "", "text", "required") +
        select("Progression", "status", ["open", "progress", "resolved"]
...
function sceneOptions(selected = "", blank = "Aucune scène") {
    return (option("", blank, selected) +
        project()
            .chapters.flatMap((c) => c.scenes.map((s) => option(s.id, `${c.title} / ${s.title}`, selected)))
            .join(""));
}
function editNote(id) {
    const p = project(), n = p.notes.find((x) => x.id === id);
    openDialog(n ? "Une page du carnet" : "Une nouvelle note", field("Titre", "title", n?.title ?? "", "text", "required") +
        select("Qui peut recevoir cette note ?", "scope", option("author", "Auteur uniquement", n?.scope ?? "author") +
            option("reader", "Lecteur", n?.scope) +
            p.entities
                .filter((en) => en.kind === "character")
                .map((en) => option(`character:${en.id}`, en.name, n?.scope))
                .join("")) +
        select("Pour le lecteur : accessible à partir de…", "revealedIn", sceneOptions(n?.revealedIn, "Dès le début")) +
        area("Note (Markdown accepté)", "text", n?.text ?? "", 'rows="8" required') +
        (n
            ? btn("Supprimer la note", "delete-note", "danger quiet", `data-id="${id}"`)
            : ""), "Enregistrer", (data) => {
        modal.close();
        mutate(() => {
            const value = {
            ...
            if (n)
                ...
                p.notes.push({ id: uid("NOTE"), ...value });
        });
    });
}
function readRows() {
    eventRows = [...modal.querySelectorAll(".change-row")].map((row) => Object.fromEntries([
...
function editEvent(id) {
    const p = project(), ev = p.events.find((x) => x.id === id), s = currentScene();
    if (!p.entities.length) {
        flash("Créez d’abord une fiche dans Univers pour décrire le sujet du fait.");
        route = "world";
        render();
        return;
    }
    const asRow = (ch, layer, holder = "") => ({
        layer,
        subject: ch.subject,
    ...
    openDialog(ev ? "Un événement du récit" : "Un nouvel événement", `<div class="notice">Un événement change le monde une seule fois. Déclarez séparément ce qu’un personnage croit et ce qui est révélé au lecteur.</div><div class="form-grid">${field("Titre", "title", ev?.title ?? "", "text", "required")}${field("Moment dans l’histoire", "storyTime", ev?.storyTime ?? s?.storyTime ?? 1, "number", 'required step="any"')}${select("Statut éditorial", "status", ["draft", "canon", "plan"].map((st) => option(st, statuses[st], ev?.status ?? "draft")).join(""))}${select("Texte source", "source", sceneOptions(ev ? (ev.source?.scene ?? "") : s?.id, "Déclaration indépendante"))}${select("Révélation au lecteur dans…", "revealedIn", sceneOptions(ev ? (ev.revealedIn ?? "") : s?.id, "Non révélé"))}</div><label class="checkbox milestone-choice"><input name="milestone" type="checkbox" ${ev?.milestone ? "checked" : ""}> ◆ Jalon marquant : afficher sur la frise macro</label>${arcChoices(ev?.arcIds)}<datalist id="truth-values"><option value="true">Oui / vrai</option><option value="false">Non / faux</option></datalist><div id="event-rows">${rowMarkup()}</div>${btn(icon("plus") + "Ajouter un changement", "add-row", "secondary")}<p class="muted small">Les valeurs true, false et les nombres sont reconnus. Le texte libre reste du texte. Une fin temporaire retire l’information, sans restaurer l’ancienne valeur.</p>${ev ? btn("Supprimer cet événement", "delete-event", "danger quiet", `data-id="${id}"`) : ""}`, "Enregistrer l’événement", (data) => {
        ...
        const readerReveals = [];
        if (!eventRows.length)
            throw Error("Ajoutez au moins un changement.");
        for (const row of eventRows) {
            if (!row.subject || !row.property.trim())
                throw Error("Chaque changement doit avoir un sujet et une propriété.");
            let value = row.value;
            try {
            ...
            if (row.until !== "") {
                if (row.mode !== "set")
                    throw Error("Pour une information temporaire, utilisez l’opération Attribuer une valeur.");
                if (row.layer === "reader")
                    throw Error("Une révélation se termine par une nouvelle révélation explicite (opération Terminer).");
            ...
            else if (row.layer === "reader")
                readerReveals.push(ch);
            else
                changes.push(ch);
        }
        const sourceId = String(data.get("source")), sourceChapter = p.chapters.find((c) => c.scenes.some((sc) => sc.id === sourceId)), sourceScene = sourceChapter?.scenes.find((sc) => sc.id === sourceId);
        if (String(data.get("status")) === "draft" && !sourceScene)
            throw Error("Un événement de brouillon doit être rattaché à une scène.");
        const value = {
            id: ev?.id ?? uid("EV"),
            title: String(data.get("title")),
            storyTime: time,
        ...
        mutate(() => {
            if (ev)
                Object.assign(ev, { source: undefined }, value);
            else
                p.events.push(value);
        });
        flash(value.status === "canon"
...
async function importProject() {
    ...
    input.addEventListener("change", async () => {
        ...
        try {
            ...
            flash(`${incoming.length} histoire(s) importée(s) dans de nouvelles copies.`);
        }
        catch (error) {
            flash(`Import impossible : ${error.message}`);
        }
    });
    input.click();
}
document.addEventListener("click", (event) => {
    ...
    const actions = {
        ...
        "move-chapter": () => {
            modal.close();
            mutate(() => move(p.chapters, p.chapters.findIndex((c) => c.id === id), Number(el.dataset.direction)));
        },
        "move-scene": () => {
            const c = p.chapters.find((c) => c.scenes.some((s) => s.id === id));
            modal.close();
            mutate(() => move(c.scenes, c.scenes.findIndex((s) => s.id === id), Number(el.dataset.direction)));
        },
        "delete-project": () => {
            const target = library.projects.find((p) => p.id === id);
            confirm("Supprimer cette histoire ?", `${e(target.title)} et tous ses textes seront retirés de ce navigateur. Exportez une sauvegarde avant de continuer.`, () => mutate(() => {
        ...
        "delete-entity": () => {
            const referenced = p.events.some((ev) => [
                ...
                ev.beliefs?.some((b) => b.holder === id)) || p.notes.some((n) => n.scope === `character:${id}`);
            if (referenced) {
                flash("Cette fiche est utilisée dans la continuité ou les notes. Modifiez ces références avant de la supprimer.");
                return;
            }
            modal.close();
        ...
        "delete-event": () => {
            modal.close();
            confirm("Supprimer cet événement ?", "Ses effets ne seront plus appliqués dans les contextes.", () => mutate(() => {
                p.events = p.events.filter((ev) => ev.id !== id);
                p.chapters
                    .flatMap((c) => c.scenes)
                    .forEach((s) => (s.presentations = s.presentations?.filter((pr) => pr.event !== id)));
            }));
        },
        "delete-arc": () => {
            modal.close();
        ...
        "delete-note": () => {
            modal.close();
            confirm("Supprimer cette note ?", "Cette note sera retirée du carnet et des contextes.", () => mutate(() => (p.notes = p.notes.filter((n) => n.id !== id))));
        },
        backup: () => {
            save();
        ...
        manuscript: () => download(`${filename()}.md`, manuscript(p), "text/markdown;charset=utf-8"),
        "context-export": () => {
            const s = currentScene(), c = currentChapter();
            if (!s || !c)
                return;
            const result = compile(p, {
                chapter: c.id,
            ...
            openDialog("Exporter le contexte", `<p>Vue ${view === "writer" ? "auteur" : view === "reader" ? "lecteur" : "personnage"} · ${e(s.title)}</p><pre class="context-preview">${e(result.markdown)}</pre>${select("Format", "format", option("md", "Markdown (.md)") + option("json", "JSON (.json)"))}`, "Télécharger", (data) => {
                const json = data.get("format") === "json";
                download(`context-${s.id}-${view}.${json ? "json" : "md"}`, json ? JSON.stringify(result.context, null, 2) : result.markdown, json ? "application/json" : "text/markdown;charset=utf-8");
                modal.close();
            });
        },
        report: () => {
            const report = p.chapters.flatMap((c) => c.scenes.map((s) => ({
                chapter: c.id,
                scene: s.id,
                issues: compile(p, { chapter: c.id, scene: s.id, includeDraft: true })
                    .issues,
            })));
            download("continuity-report.json", JSON.stringify({
                story: p.title,
        ...
        import: () => void importProject(),
            ...
            try {
                ...
                if (backup)
                    download("metamachia-secours.json", backup);
                else
                    flash("Pas encore de sauvegarde précédente disponible.");
            }
            catch {
...
document.addEventListener("input", (event) => {
    const el = event.target, p = project();
    if (!p)
        return;
    ...
    if (el.id === "world-search") {
        ...
        render();
        const input = document.querySelector("#world-search");
        input.focus();
...
document.addEventListener("change", (event) => {
    ...
    if (el.id === "scene-complete") {
        ...
        if (s)
            mutate(() => {
                s.status = el.checked ? "complete" : "draft";
            });
    }
});
