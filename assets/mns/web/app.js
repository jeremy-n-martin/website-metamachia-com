import { linetteTimeline } from "../src/linette-example.js?v=6883f063ff99";
import { KEY, MAX_DEPTH, LABELS, HINTS, IDEAS, RELATIONS, id, emptyTimeline, demoTimeline, entries, insertLoop, deleteLoop, deleteCharacter, deleteGroup, outline, passages, markdown, warnings, parseTimeline, chronology, duration, timeLabel, normalizeName, } from "../src/timeline.js?v=6883f063ff99";
import { effectDescription, choiceField, inventoryFields, propertyFields, worldHtml, consequenceHtml, } from "./state-ui.js?v=6883f063ff99";
const app = document.querySelector("#app");
const modal = document.querySelector("#modal");
const toast = document.querySelector("#toast");
const e = (v) => String(v ?? "").replace(/[&<>"']/g, (c) => ({ "&": "&amp;", "<": "&lt;", ">": "&gt;", '"': "&quot;", "'": "&#39;" })[c]);
const button = (action, text, data = "", cls = "") => `<button type="button" class="${cls}" data-action="${action}" ${data}>${text}</button>`;
let project = emptyTimeline(), previous = "", blocked = false, storageAvailable = true, startup = "";
try {
    previous = localStorage.getItem(KEY) || "";
    if (previous)
        project = parseTimeline(previous);
}
catch {
    blocked = true;
    startup =
...
let selected = entries(project)[0]?.block.id || "", position = 0, playing = false, speed = 4, direction = 1, lastTime = 0, frame = 0, lastCount = -1;
let history = [], future = [], legacy = "", zoom = 1;
...
const name = (person) => project.characters.find((c) => c.id === person)?.name ||
    "Personnage supprimé";
const selectedBlock = () => entries(project).find((x) => x.block.id === selected)?.block;
const announce = (text) => {
...
function persist() {
    if (blocked || !storageAvailable)
        return false;
    try {
        if ((localStorage.getItem(KEY) || "") !== previous) {
            blocked = true;
            announce("Une autre fenêtre a modifié la frise. Exportez vos modifications puis rechargez.");
            return false;
        }
        ...
        storageAvailable = false;
        announce("Stockage plein ou indisponible. Exportez la frise : vos modifications restent en mémoire.");
        return false;
    }
}
function stop() {
...
function render() {
  ...
  <div class="canvas-scroll"><div class="canvas" style="--density:${zoom}"><div class="canvas-intro"><span class="eyebrow">UNE HISTOIRE, PLUSIEURS PROFONDEURS</span><h2>Dépliez le fil.</h2><p>Quatre temps pour avancer. Des boucles pour approfondir.</p><div class="legend">${Object.entries(LABELS)
    ...
    lastCount = -1;
    updateReader();
    document.querySelector(".canvas").style.minWidth =
...
function updateReader(follow = false) {
    ...
    const surface = document.querySelector(".timeline-surface"), cursor = document.querySelector("#reading-cursor");
    if (!surface || !cursor)
        return;
    const marks = [
...
function play() {
    ...
    const tick = (now) => {
        if (!playing)
            return;
        position = Math.max(0, Math.min(total, position + (direction * Math.min(now - lastTime, 100)) / (speed * 1000)));
        lastTime = now;
        updateReader(true);
...
const field = (label, key, value = "", required = false, max = 100) => `<label>${label}<input name="${key}" value="${e(value)}" maxlength="${max}" ${required ? "required" : ""}></label>`;
const formEnd = (text = "Enregistrer") => `<div class="form-actions">${button("close", "Annuler")}<button class="primary" type="submit">${text}</button></div></form>`;
const options = (items, value = "") => items
    .map((i) => `<option value="${i.id}" ${i.id === value ? "selected" : ""}>${e(i.name)}</option>`)
    .join("");
function personForm(personId) {
    const c = project.characters.find((c) => c.id === personId);
    show(c ? "Modifier le personnage" : "Un nouveau personnage", `<form data-form="person" data-id="${c?.id || ""}">${field("Nom", "name", c?.name, true)}${field("Que veut ce personnage ?", "goal", c?.goal, false, 20000)}<label>Couleur<input type="color" name="color" value="${c?.color || "#578780"}"></label><label class="check"><input type="checkbox" name="present" ${!c || c.present ? "checked" : ""}>Existe déjà à l’état zéro</label><p class="muted">Pour une naissance future, décochez puis ajoutez « Entrée dans le récit » au bloc concerné.</p>${c ? button("delete-person", "Supprimer le personnage", `data-id="${c.id}"`, "danger") : ""}${formEnd()}`);
}
function groupForm(groupId) {
    const g = project.groups.find((g) => g.id === groupId);
    show(g ? "Modifier le groupe à l’état zéro" : "Un groupe ou une faction", `<form data-form="group" data-id="${g?.id || ""}">${field("Nom", "name", g?.name, true)}<label>Nature<select name="kind"><option value="groupe">Groupe</option><option value="faction" ${g?.kind === "faction" ? "selected" : ""}>Faction</option></select></label><fieldset><legend>Membres à l’état zéro</legend>${project.characters.map((c) => `<label class="check"><input type="checkbox" name="members" value="${c.id}" ${g?.members.includes(c.id) ? "checked" : ""}>${e(c.name)}</label>`).join("")}</fieldset>${g ? button("delete-group", "Supprimer le groupe", `data-id="${g.id}"`, "danger") : ""}${formEnd()}`);
}
...
function confirmAction(title, text, action, target = "") {
    show(title, `<p>${e(text)}</p><p class="muted">Vous pourrez annuler cette opération depuis la barre supérieure.</p><div class="form-actions">${button("close", "Conserver")}${button(action, "Confirmer", `data-id="${target}"`, "danger")}</div>`);
}
...
document.addEventListener("click", (event) => {
    const target = event.target, b = target.closest("[data-action]");
    if (!b) {
        const card = target.closest("[data-select]");
        if (card)
            selectBlock(card.dataset.select);
        return;
    }
    const action = b.dataset.action, targetId = b.dataset.id;
    if (action === "close") {
        modal.close();
        return;
    }
    ...
    if (action === "use-idea") {
        const block = selectedBlock();
        if (block.text)
            confirmAction("Remplacer le texte ?", "Votre texte sera remplacé par l’amorce sélectionnée.", "confirm-idea");
        else
            change(() => {
    ...
    if (action === "delete-loop")
        confirmAction("Supprimer cette boucle ?", "Ses quatre blocs, leurs textes, leurs changements et toutes leurs sous-boucles seront supprimés.", "confirm-loop", targetId);
    if (action === "confirm-loop") {
        modal.close();
        change(() => deleteLoop(project, targetId));
    }
    if (action === "delete-person")
        confirmAction("Supprimer ce personnage ?", "Ses liens, appartenances et interventions seront retirés. Son nom peut rester dans les textes que vous avez écrits.", "confirm-person", targetId);
    if (action === "confirm-person") {
        modal.close();
        change(() => deleteCharacter(project, targetId));
    }
    if (action === "delete-group")
        confirmAction("Supprimer ce groupe ?", "Le groupe et ses changements d’appartenance seront retirés.", "confirm-group", targetId);
    if (action === "confirm-group") {
        modal.close();
        change(() => deleteGroup(project, targetId));
    }
    if (action === "delete-link") {
        change(() => (project.links = project.links.filter((l) => l.id !== targetId)));
        baselineForm();
    }
    if (action === "loop-title") {
        const loop = entries(project).find((x) => x.loop.id === targetId)?.loop;
        if (loop)
            show("Nommer la boucle", `<form data-form="loop" data-id="${targetId}">${field("Titre", "title", loop.title, true, 200)}${formEnd()}`);
    }
    if (action === "undo" || action === "redo") {
        stop();
        const source = action === "undo" ? history : future, dest = action === "undo" ? future : history, next = source.pop();
        if (next) {
    ...
    if (action === "confirm-linette") {
        ...
        announce("Exemple Linette chargé. La lecture commence au départ dans la forêt ; les horaires attestés figurent dans les blocs.");
    }
    if (action === "family-demo")
        confirmAction("Charger l’exemple ?", "La frise courante sera remplacée. Sauvegardez-la d’abord si vous souhaitez la conserver au-delà de cette session.", "confirm-demo");
    ...
    if (action === "import") {
        ...
        input.onchange = async () => {
            ...
            try {
                ...
                show("Remplacer la frise par cette archive ?", `<p>${e(imported.title)} · ${entries(imported).length} blocs. Exportez la frise actuelle avant de la remplacer.</p><div class="form-actions">${button("close", "Annuler")}<button id="confirm-import" class="primary">Importer</button></div>`);
                modal
...
document.addEventListener("input", (event) => {
    ...
    if (el.dataset.field) {
        ...
        card.querySelector(".block-excerpt").textContent = (b.text || outline(project, b)).slice(0, 185);
        card
...
document.addEventListener("drop", (event) => {
    ...
    if (!project.characters.some((c) => c.id === person))
        return;
    selected = card.dataset.select;
    change(() => {
        const b = selectedBlock();
        if (!b.cast.includes(person))
            b.cast.push(person);
    });
    announce("Personnage ajouté. Choisissez une direction ou déclarez un changement de lien.");
});
document.addEventListener("keydown", (event) => {
    if ((event.key === "Enter" || event.key === " ") &&
        event.target.matches("[data-select]")) {
        event.preventDefault();
        selectBlock(event.target.dataset.select);
    }
});
window.addEventListener("storage", (event) => {
    if (event.key === KEY && event.newValue !== previous) {
        ...
        blocked = true;
        render();
        announce("La frise a changé dans un autre onglet. Exportez votre version puis rechargez pour éviter de l’écraser.");
    }
});
