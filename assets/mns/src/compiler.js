import { key, } from "./model.js?v=528a9284a674";
import { sceneArcs, sceneLocation } from "./narrative.js?v=528a9284a674";
...
export function validateStory(story) {
    ...
    for (const event of story.events) {
        ...
        if (event.source) {
            const chapter = story.chapters.find((c) => c.id === event.source.chapter);
    ...
    for (const note of story.notes) {
        if (note.scope.startsWith("character:") &&
...
function reconstruct(operations, target, issues, label) {
    const values = new Map();
    const owners = new Map();
    const seenAt = new Map();
    const conflicts = new Set();
    const queue = operations.flatMap((op) => op.change.until === undefined
        ? [op]
    ...
    for (const op of queue) {
        ...
        if (mode === "end")
            ...
            values.set(k, op.change.value);
    }
    return values;
}
export function compile(story, request) {
    ...
    const operations = (field) => unique.flatMap((e) => (e[field] ?? []).map((change, i) => ({
        time: e.storyTime,
        id: `${e.id}_${field}_${i}`,
        change: field === "beliefs"
            ? {
                ...change,
                subject: `${change.holder}:${change.subject}`,
            }
            : change,
    })));
    ...
    if (request.includeDraft)
        scenes
            ...
            .forEach((s) => (s.draftChanges ?? []).forEach((change, i) => factOps.push({ time: s.storyTime, id: `${s.id}_draft_${i}`, change })));
    state.facts = reconstruct(factOps, scene.storyTime, issues, "Monde");
    state.beliefs = reconstruct(operations("beliefs"), scene.storyTime, issues, "Croyances");
    ...
    for (const event of unique) {
        const positions = event.revealedIn
            ...
            : scenes.flatMap((s, i) => s.presentations?.some((p) => p.event === event.id) ? [i] : []);
        const position = positions.filter((i) => i >= 0).sort((a, b) => a - b)[0];
        if (position !== undefined)
            (event.readerReveals ?? []).forEach((change, i) => revealOps.push({
                time: position,
    ...
    state.applied = unique
        ...
        .map((e) => e.id);
    const characterKnowledge = Object.fromEntries([...state.beliefs]
