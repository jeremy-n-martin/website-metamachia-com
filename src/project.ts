...
import { validateStory } from "./compiler.js";
...
export const projectWords = (p: Project) =>
  p.chapters.reduce(
    (sum, c) => sum + c.scenes.reduce((n, s) => n + wordCount(s.text ?? ""), 0),
    0,
  );
export const newScene = (time = 1): Scene => ({
  id: uid("SC"),
  ...
  p.entities = [
    {
      ...
      kind: "character",
      description:
        "Messagère des songes. Manteau vert, doigts tachés d’encre. Elle croit que toute lettre finit par trouver son destinataire.",
    },
    {
      id: "KING",
      name: "Le roi Éloi",
      kind: "character",
      description:
        "Souverain de la cité. Son sceau est une hirondelle d’argent.",
    },
    {
      id: "CITY",
      ...
      kind: "place",
      description:
        "Des toits de cuivre, des canaux silencieux et une poste qui n’ouvre qu’à minuit.",
    },
    {
      id: "LETTER",
      ...
      kind: "object",
      description:
        "Une enveloppe scellée à la cire blanche. L’encre scintille dans l’obscurité.",
    },
  ];
  p.chapters = [
    {
      ...
      brief:
        "Installer la cité et le métier de Linette. Une lettre inattendue bouleverse sa tournée.",
      scenes: [
        {
          id: "SC_A",
  ...
  p.arcs = [
    {
      ...
      description:
        "Une lettre royale apparaît après la mort du roi. Suivre les indices laissés par la plume.",
      resolution:
        "La promesse du roi explique la lettre. Linette choisira de la délivrer.",
      status: "open",
    },
  ];
  return p;
...
function str(v: unknown, path: string, fallback?: string): string {
  ...
  if (typeof v !== "string") throw Error(`${path} : texte attendu.`);
  return v;
}
...
function list(v: unknown, path: string): unknown[] {
  ...
  if (!Array.isArray(v)) throw Error(`${path} : liste attendue.`);
  return v;
}
function one<const T extends string>(v: unknown, choices: T[], fallback: T): T {
  if (v === undefined) return fallback;
  if (!choices.includes(v as T)) throw Error(`Valeur invalide : ${String(v)}`);
  return v as T;
}
function change(v: unknown): FactChange {
...
export function parseProject(text: string): Project {
  if (text.length > 10_000_000) throw Error("Le fichier dépasse 10 Mo.");
  const o = obj(parseJson(text), "Projet");
  if (o.format !== "mns-web" || o.version !== 1)
    throw Error(
      "Format non reconnu. Importez une sauvegarde MNS (.json), version 1.",
    );
  for (const name of ["entities", "chapters", "events", "notes", "arcs"])
    if (!Array.isArray(o[name])) throw Error(`Liste requise : ${name}`);
  const p: Project = {
    format: "mns-web",
    version: 1,
    id: str(o.id, "ID"),
...
export class LibraryStore {
  ...
  load(): Library {
    if (!this.previous) return { projects: [], activeId: null };
    const o = obj(JSON.parse(this.previous), "Bibliothèque");
    return {
      projects: list(o.projects, "Histoires").map((v) =>
        parseProject(JSON.stringify(v)),
      ),
  ...
  save(library: Library): void {
    if (this.storage.getItem(STORAGE_KEY) !== this.previous)
      throw Error(
        "La bibliothèque a changé dans un autre onglet. Exportez votre travail, puis rechargez la page.",
      );
